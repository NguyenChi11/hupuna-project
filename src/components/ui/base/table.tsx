          ...
          {pageNumbers.map((page, index) =>
            page === "..." ? (
              ...
              <button
                ...
                className={`px-3 py-1 rounded ${current === page
        ...
        <table className="min-w-full text-md bg-white">
          <thead className={`${classNameHead} bg-[#99b4d4] text-gray-800 font-semibold text-sm`}>
            <tr>
              ...
              {/* STT ================ */}
              {isSTT && (
                <th className={`border border-gray-400 text-center p-1 w-[50px]`}>
                  STT
                </th>
              )}
              {/* Column ================= */}
          ...
          <tbody className="text-md text-gray-600">
            {loading ? (
              ...
              paginatedData.map((record, rowIndex) => {
                ...
                const isExpanded = expandedRowKey === (record[rowKey] as string | number);

                return (
                  <React.Fragment key={key}>
                    <tr
                      key={key}
                      className={`text-md border-b border-gray-200 cursor-pointer -z-0 ${isSelected ? "bg-blue-50" : ""} ${classNameRow} `}
                      ...
                      onMouseEnter={(e) => {
                        (e.currentTarget as HTMLTableRowElement).style.filter = "brightness(0.95)";
                      }}
                      onMouseLeave={(e) => {
                        (e.currentTarget as HTMLTableRowElement).style.filter = "brightness(1)";
                      }}
                    >
                      {/* Checkbox ============= */}
                      {rowSelection && (
